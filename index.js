...
function create(key,value,path,ttl)
{
    ...
    try{
        ...
        fs.access(path+'user.txt',fs.F_OK,err=>{
            ...
            //If file already exists
                ...
                console.log('yes')
                fs.readFile('user.txt','utf-8',(err,dat)=>{
                    if(err){
                        throw err;
                    }
                    var json = []
...
function read(key){
    var data = fs.readFileSync('user.txt', 'utf-8', (err, data) => {
        if (err) {
            throw err;
        }      
        // parse JSON object
...
create('abcdf','{"id":5,"name":"Harsh1","age":22}','.\\',5)
//deleteKey('abc')
